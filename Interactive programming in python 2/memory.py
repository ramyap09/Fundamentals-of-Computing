...
def reset():
    ...
    """
    global exposed, x, text_id, nlist, state, turns, lst
    exposed = [False]*16
    x = []
...
def click(event):
    ...
    """
    global exposed, state, x, turns, lst, nlist
    ...
    try:
        c = canvas1.find_closest(event.x,event.y)[0]
        ...
        if state == 3 and  nlist[0] == nlist[1] :
            del text_id[0:2]
            del nlist[0:2]
            del x[0:2]
            state = 1
        ...
        pass  # This portion is executed when you click on the already displayed number on the card.
